CREATE OR REPLACE TABLE `{{ YOUR_PROJECT }}.{{ YOUR_DATASET }}.communication_planning_{{ F|D }}_{{ YYYYMMDD }}` (
	...
	, sending_sys_cd ARRAY<STRING> NOT NULL  OPTIONS(description="the systems code which sent the message (e.g.: 'NEWSLETTER', 'E-SHOP', ...)")
	...
	, thread_conditions ARRAY<STRUCT<
		condition_id STRING NOT NULL OPTIONS(description="")
		, type STRING NOT NULL OPTIONS(description="")
		, source STRING NOT NULL OPTIONS(description="")
		, operator STRING NOT NULL OPTIONS(description="")
		, values ARRAY<STRING> NOT NULL OPTIONS(description="")
	...
	, channel_names ARRAY<STRING> NOT NULL OPTIONS(description="the channels (name) of the communication: 'email', ...")

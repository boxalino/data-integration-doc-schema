CREATE OR REPLACE TABLE `{{ YOUR_PROJECT }}.{{ YOUR_DATASET }}.doc_language_{{ F|D }}_{{ YYYYMMDD }}`  (
	...
	, country_code STRING OPTIONS(description="optional, needed only to differentiate two diffrent ways to address the same language (e.g.: 'ch', 'de', ...), if used, the language used in other tables must be of the format '[country_code]-[language]', like 'ch-de' and 'de-de'")
